  ...
  const [fetchUserAuthState, sendToUserAuthMachine] = useMachine(fetchAuthUserMachine, {
    actions: {
      fetchUserAuth: (
        ...
        if (event.payload.byToken) {
          ...
          UserRepoClass.getUserByToken().then(
            ...
            (err: FetchServerErrorResponse) => {
              logger.log('User auth with token failed:', err.errors.message);
              sendToUserAuthMachine({
                type: 'REJECT_AUTH',
                err: null,
                attRef: false,
        ...
        UserRepoClass.authUser(event.payload.username, event.payload.password).then(
          (res: UserAuthProps) => {
      ...
      refreshToken: (ctx, event: { type: 'RETRY'; payload: { refreshToken: TokenPayload } }) => {
        ...
        UserRepoClass.refreshToken(event.payload.refreshToken).then(
          ...
          (err: FetchServerErrorResponse) => {
            logger.log('err while trying to refresh token:', err.errors.message);
            sendToUserAuthMachine({
              type: 'REJECT_AUTH',
              err: null,
              attRef: true,
