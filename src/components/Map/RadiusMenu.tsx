      ...
      <div className="relative w-full">
        <Listbox value={radius} disabled={mapDataFetchState.value === 'pending'} onChange={changeHandler}>
          <Listbox.Button
            name="select-radius"
            id="select-radius-id"
            className={`relative w-full p-4 text-left bg-no-repeat bg-clip-padding border-none rounded-xl cursor-pointer transition-colors focus:border-acid-green focus:outline-none ${
              ...
              {rangeOptionList.map((option: number) => (
                <Listbox.Option
                  ...
                  className={({ active, selected }) =>
                    `relative px-1.5 py-2.5 pr-10 pl-9 cursor-pointer transition-colors ${selected ? 'bg-bright-seaweed' : ''} ${active && !selected ? 'bg-hovered-seaweed' : ''}`
                  }>
                  <span>{option} km</span>
                </Listbox.Option>
              ))}
